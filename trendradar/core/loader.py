...
def _load_phd_jobs_config(config_data: Dict) -> Dict:
    """加载 PhD 职位配置(支持环境变量覆盖)。"""
    phd = config_data.get("phd_jobs", {})
    countries = phd.get("countries") or {}

    # 职位类型:默认 phd+postdoc+doctoral
    position_types = phd.get("position_types")
    if not position_types or not isinstance(position_types, list):
        position_types = ["phd", "postdoc", "doctoral"]
    env_types = os.environ.get("PHD_JOBS_POSITION_TYPES", "").strip()
    if env_types:
        position_types = [t.strip().lower() for t in env_types.split(",") if t.strip()]

    return {
        "ENABLED": (
            _get_env_bool("PHD_JOBS_ENABLED")
            if _get_env_bool("PHD_JOBS_ENABLED") is not None
            else phd.get("enabled", False)
        ),
        "PUSH_ONLY": (
            _get_env_bool("PHD_JOBS_PUSH_ONLY")
            if _get_env_bool("PHD_JOBS_PUSH_ONLY") is not None
            else phd.get("push_only", False)
        ),
        "MAX_PAGES": _get_env_int_or_none("PHD_JOBS_MAX_PAGES") or phd.get("max_pages", 3),
        "REQUEST_INTERVAL_MS": phd.get("request_interval_ms", 800),
        "POSITION_TYPES": position_types,
        "COUNTRIES": [
            c for c in ("Norway", "Netherlands", "Sweden", "Finland", "Denmark", "Italy") if c in countries
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # PhD 职位配置
    config["PHD_JOBS"] = _load_phd_jobs_config(config_data)
