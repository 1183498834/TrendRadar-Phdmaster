...
class NewsAnalyzer:
    ...
    def _merge_phd_standalone(self, standalone_data, phd_standalone):
        """把 PhD 职位 feed 合并进独立展示区数据。"""
        if not phd_standalone or not phd_standalone.get("rss_feeds"):
            return standalone_data
        if not standalone_data:
            return phd_standalone
        merged = {
            "platforms": standalone_data.get("platforms", []) + phd_standalone.get("platforms", []),
            "rss_feeds": standalone_data.get("rss_feeds", []) + phd_standalone.get("rss_feeds", []),
        }
        return merged
    ...
    def _execute_mode_strategy(
        ...
        rss_new_urls: Optional[set] = None,
        phd_standalone: Optional[Dict] = None,
    ) -> Optional[str]:
        ...
        if self.report_mode == "current":
            ...
            if analysis_data:
                ...
                standalone_data = self._merge_phd_standalone(standalone_data, phd_standalone)
    ...
    def _crawl_phd_jobs(self) -> Tuple[Optional[Dict], List[Dict], List[Dict]]:
        """抓取 PhD 职位,返回 (standalone_data, new_jobs, all_jobs)。

        - standalone_data: 职位 → 独立展示区形状(每国一个 feed)
        - new_jobs: 相比上次快照新增的职位(用于推送)
        - all_jobs: 本次全部职位(用于保存快照)
        未启用或失败时返回 (None, [], [])。
        """
        phd_cfg = self.ctx.phd_jobs_config
        if not phd_cfg.get("ENABLED"):
            print("[phd] PhD 职位抓取未启用(PHD_JOBS.enabled=false),跳过")
            return None, [], []

        from trendradar.crawler.phd_jobs import fetch_phd_jobs, build_standalone
        from trendradar.crawler.phd_jobs import snapshot as phd_snapshot

        countries = phd_cfg.get("COUNTRIES") or []
        if not countries:
            print("[phd] 未配置目标国家,跳过")
            return None, [], []

        try:
            all_jobs = fetch_phd_jobs(phd_cfg)
        except Exception as exc:
            print(f"[phd] 职位抓取失败: {exc}")
            return None, [], []

        if not all_jobs:
            print("[phd] 无职位,跳过推送与快照更新")
            return None, [], []

        # 增量对比:计算新增职位
        previous_keys = phd_snapshot.load_previous(phd_cfg)
        new_jobs = phd_snapshot.detect_new(all_jobs, previous_keys)
        print(
            f"[phd] 对比上次: 新增 {len(new_jobs)} 条"
            f"(历史 {len(previous_keys)} 条)"
        )

        # 保存快照(供下次对比)
        snapshot_path = phd_snapshot.save(all_jobs, phd_cfg)
        print(f"[phd] 快照已保存: {snapshot_path}")

        # 组装独立展示区数据(仅新增职位)
        standalone = build_standalone(new_jobs, countries)
        return standalone, new_jobs, all_jobs

    def _run_phd_only(self) -> None:
        """仅推送 PhD 职位模式:不抓热榜/RSS,不跑 AI,只爬职位并推送。"""
        print("=" * 60)
        print("[phd] 仅推送 PhD 职位模式（push_only=true）,跳过热榜/RSS/AI 分析")
        print("=" * 60)

        phd_cfg = self.ctx.phd_jobs_config
        if not phd_cfg.get("ENABLED"):
            print("[phd] PhD 职位抓取未启用(PHD_JOBS.enabled=false),退出")
            return

        # 1. 抓取职位并做增量对比
        standalone, new_jobs, all_jobs = self._crawl_phd_jobs()
        if not new_jobs:
            print("[phd] 本次无新增职位,不推送")
            return

        print(
            f"[phd] 本次新增 {len(new_jobs)} 条职位,准备推送"
        )

        # 2. 检查通知渠道
        if not self._has_notification_configured():
            print("⚠️ 警告: 未配置任何通知渠道，将不发送推送")
            return

        # 3. 构造报告数据(空热榜/RSS,仅职位在独立展示区)
        report_data = {
            "stats": [],
            "new_titles": [],
            "failed_ids": [],
        ...
        # 4. 生成 HTML 报告(供邮件渠道使用)
        html_file = self._generate_phd_html(new_jobs, phd_cfg)

        # 5. 分发到所有渠道
        dispatcher = self.ctx.create_notification_dispatcher()
        results = dispatcher.dispatch_all(
            report_data=report_data,
            report_type="PhD 职位推送",
            update_info=None,
            proxy_url=self.proxy_url,
            mode="daily",
            html_file_path=html_file,
        ...
        for channel, ok in results.items():
            print(f"[推送] {channel}: {'✅ 成功' if ok else '❌ 失败'}")

        if any(results.values()):
            print(f"[phd] 推送完成: 新增 {len(new_jobs)} 条职位")

    def _generate_phd_html(self, jobs, phd_cfg) -> Optional[str]:
        ...
        try:
            ...
            from pathlib import Path
            output_dir = Path("output") / "html" / self.ctx.format_date()
            output_dir.mkdir(parents=True, exist_ok=True)
            html_path = output_dir / f"phd_jobs_{self.ctx.format_time()}.html"
            html_path.write_text(html_content, encoding="utf-8")
            print(f"[phd] HTML 报告已生成: {html_path}")
            return str(html_path)
        except Exception as exc:
            print(f"[phd] 生成 HTML 报告失败: {exc}")
            return None
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # 仅推送 PhD 职位模式:跳过热榜/RSS/AI
            if self.ctx.phd_jobs_config.get("PUSH_ONLY"):
                self._run_phd_only()
                return
            ...
            # 抓取 PhD 职位（如果启用），组装独立展示区数据
            phd_standalone, phd_new_jobs, phd_all_jobs = self._crawl_phd_jobs()

            # 执行模式策略，传递 RSS 数据与 PhD 职位用于合并推送
